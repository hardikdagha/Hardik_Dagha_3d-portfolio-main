import { useEffect, useState } from "react";
import "./styles/Loading.css";
import { useLoading } from "../context/LoadingProvider";

import Marquee from "react-fast-marquee";
import { profile } from "../data/profile";

const Loading = ({ percent }: { percent: number }) => {
  const { setIsLoading } = useLoading();
  const [loaded, setLoaded] = useState(false);
  const [clicked, setClicked] = useState(false);

  useEffect(() => {
    if (percent < 100 || loaded) return;

    const loadedTimer = window.setTimeout(() => {
      setLoaded(true);
      setClicked(true);
    }, 250);

    return () => {
      window.clearTimeout(loadedTimer);
    };
  }, [loaded, percent]);

  useEffect(() => {
    if (!loaded) return;

    const failSafeTimer = window.setTimeout(() => {
      setIsLoading(false);
    }, 1400);

    const introTimer = window.setTimeout(async () => {
      try {
        const module = await import("./utils/initialFX");
        module.initialFX?.();
      } catch (error) {
        console.error("Initial intro animation failed:", error);
      } finally {
        window.setTimeout(() => {
          window.clearTimeout(failSafeTimer);
          setIsLoading(false);
        }, 250);
      }
    }, 200);

    return () => {
      window.clearTimeout(introTimer);
      window.clearTimeout(failSafeTimer);
    };
  }, [loaded, setIsLoading]);

  function handleMouseMove(e: React.MouseEvent<HTMLElement>) {
    const { currentTarget: target } = e;
    const rect = target.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    target.style.setProperty("--mouse-x", `${x}px`);
    target.style.setProperty("--mouse-y", `${y}px`);
  }

  return (
    <>
      <div className="loading-header">
        <a href="#" className="loader-title" data-cursor="disable">
          {profile.name.initials}
        </a>
        <div className={`loaderGame ${clicked && "loader-out"}`}>
          <div className="loaderGame-container">
            <div className="loaderGame-in">
              {[...Array(27)].map((_, index) => (
                <div className="loaderGame-line" key={index}></div>
              ))}
            </div>
            <div className="loaderGame-ball"></div>
          </div>
        </div>
      </div>
      <div className="loading-screen">
        <div className="loading-marquee">
          <Marquee>
            <span> Cybersecurity Analyst</span>
            <span> Security Operations</span>
            <span> Detection Engineering</span>
            <span> Incident Triage</span>
          </Marquee>
        </div>
        <div
          className={`loading-wrap ${clicked && "loading-clicked"}`}
          onMouseMove={(e) => handleMouseMove(e)}
        >
          <div className="loading-hover"></div>
          <div className={`loading-button ${loaded && "loading-complete"}`}>
            <div className="loading-container">
              <div className="loading-content">
                <div className="loading-content-in">
                  Loading <span>{percent}%</span>
                </div>
              </div>
              <div className="loading-box"></div>
            </div>
            <div className="loading-content2">
              <span>Opening</span>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Loading;

export const setProgress = (setLoading: (value: number) => void) => {
  let percent = 0;
  let intervalId: number | undefined;
  let isReleased = false;

  const setPercent = (nextValue: number) => {
    percent = Math.max(percent, Math.min(100, Math.round(nextValue)));
    setLoading(percent);
  };

  const clearTicker = () => {
    if (intervalId === undefined) return;
    window.clearInterval(intervalId);
    intervalId = undefined;
  };

  const startSlowTicker = () => {
    clearTicker();
    intervalId = window.setInterval(() => {
      if (percent >= 98) return;
      setPercent(percent + 1);
    }, 500);
  };

  const startFastTicker = () => {
    clearTicker();
    intervalId = window.setInterval(() => {
      if (percent >= 70) {
        startSlowTicker();
        return;
      }

      const rand = 2 + Math.round(Math.random() * 3);
      setPercent(percent + rand);
    }, 120);
  };

  startFastTicker();

  const hardStopTimer = window.setTimeout(() => {
    clear();
  }, 12000);

  function clear() {
    if (isReleased) return;
    isReleased = true;
    clearTicker();
    window.clearTimeout(hardStopTimer);
    setPercent(100);
  }

  function loaded() {
    return new Promise<number>((resolve) => {
      if (isReleased) {
        resolve(percent);
        return;
      }

      clearTicker();
      window.clearTimeout(hardStopTimer);
      intervalId = window.setInterval(() => {
        if (percent < 100) {
          setPercent(percent + 1);
        } else {
          isReleased = true;
          resolve(percent);
          clearTicker();
        }
      }, 8);
    });
  }

  function dispose() {
    clearTicker();
    window.clearTimeout(hardStopTimer);
  }

  return { loaded, percent, clear, dispose };
};
